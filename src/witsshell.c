...
void executeCommandsInParallel(char *parallelCommands[], const char *outputFile)
{
    int originalStdout = dup(STDOUT_FILENO); // Save the original STDOUT
    ...
    {
        // Child process
        char fullPath[MAX_PATH_LENGTH + strlen(parallelCommands[0]) + 2];
        snprintf(fullPath, sizeof(fullPath), "%s/%s", searchPath, parallelCommands[0]);
        execv(fullPath, parallelCommands);
        displayError("An error has occurred\n");
        exit(1);
    }
    else
    {
        // Parent process for parallel execution
        int status;
        waitpid(childPid, &status, 0);
    }
            ...
            else
            {
                ...
                for (int i = 0; i < parallelCommandCount; i++)
                {
                    ...
                    if (argCount > 0)
                    {
                        ...
                        if (shouldRun)
                        {
                            executeCommandsInParallel(commandArgs, outputFile);
                        }
                    }
                }
            }
